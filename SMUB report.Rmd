---
title: "Data exploration and SMUB development 1"
date: "25.01.2021"
...
data <- raw %>% 
  ...
  mutate(kerncel = as.factor(kerncel)) %>% 
  mutate(log.turnover = log10(omzet_excl_btw)) %>% 
  mutate(year.quarter = as.factor(paste0("Y:", Year, " ", "Q:0", Quarter))) %>% 
  ...
  mutate(selective.sam = case_when(day.cum <= 0.1 ~ 1,
                                   TRUE ~ 0)) %>% 
  ungroup() %>% 
  mutate(year.quarter.nace = as.factor(paste0("Y:", Year, " ", "Q:0", Quarter, "_", kerncel))) %>% 
  arrange(desc(year.quarter.nace, daysSinceEOQ)) %>%
  group_by(year.quarter.nace) %>% 
  mutate(day.rank = 1 / n()) %>% 
  mutate(day.cum = cumsum(day.rank)) %>% 
  mutate(selective.sam.nace = case_when(day.cum <= 0.1 ~ 1,
                                   TRUE ~ 0)) %>% 
  ungroup()
...
# SMUB per year.quarter and NACE

```{r}
selected.data.nace <- data %>% 
  select(y = log.turnover, z = wp, sample = selective.sam.nace, year.quarter.nace) %>%
  filter(sample == 1) %>% 
  split(.$year.quarter.nace) %>% 
  map(~summarise(., std.var.sel.yz = sqrt( var(y) * var(z) ), .groups = "drop")) %>% 
  map_df(~select(., std.var.sel.yz)) %>% 
  deframe()

data.final.nace <- data %>% 
  split(.$year.quarter.nace) %>%
  map(~select(., y = log.turnover, z = wp, sample = selective.sam.nace, year.quarter.nace)) %>%
  map2(., selected.data.nace, ~mutate(.x, z = z * .y))

smub.nace <- data.final.nace %>% 
  map(~SMUB(data = ., phi = seq(0, 5, 0.1)))

# Comp. table global stats all quarters
smub.nace.summary <- smub.nace %>% 
  map_depth(1, ~as_tibble(.$global.characteristics)) %>% 
  map_dfr(~select(., n.prop, bias, std.bias, mean.diff.z, r, true.phi, mean.diff.z), .id = "year.quarter") %>% 
  kbl() %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
